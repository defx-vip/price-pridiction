...
/**
 * @author 0mllwntrmt3
 * @title DEFX Protocol V8888 Staking Contract
 * @notice The contract that stakes the DEFX tokens through
 * buying the microlots (any amount of DEFX tokens per microlot)
 * and the staking lots (888,000 DEFX per lot), accumulates the staking
...
contract DEFXStaking is ERC20, IDEFXStaking {
    using SafeERC20 for IERC20;

    IERC20 public immutable DEFX;
    ...
    /**
     * @notice Used by the DEFX microlots holders
    ...
    /**
     * @notice Used for staking any amount of the DEFX tokens
    ...
    function buyMicroLot(uint256 amount) external {
        ...
        DEFX.safeTransferFrom(msg.sender, address(this), amount);
        emit MicroLotsAcquired(msg.sender, amount);
    }

    /**
     * @notice Used for unstaking the DEFX tokens
    ...
    function sellMicroLot(uint256 amount) external {
        ...
        microLotsTotal -= amount;
        microBalance[msg.sender] -= amount;
        DEFX.safeTransfer(msg.sender, amount);
        emit MicroLotsSold(msg.sender, amount);
    }

    /**
     * @notice Used for staking the fixed amount of 888,000 DEFX
