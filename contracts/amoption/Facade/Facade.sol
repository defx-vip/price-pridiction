...
contract Facade is Ownable {
    ...
    address public _trustedForwarder; //代理操作者
     ...
     **/
    function poolApprove(IDEFXPool pool) external {
     ...
     **/
    function unlockAll(IDEFXPool pool, uint256[] calldata optionIDs) external {
    ...
    function claimAllStakingProfits(
        IDEFXStaking[] calldata stakings,
        address account
    ) external {
        uint256 arrayLength = stakings.length;
        for (uint256 i = 0; i < arrayLength; i++) {
            IDEFXStaking s = stakings[i];
    ...
    function exercise(uint256 optionId) external {
        ...
        IDEFXPool(optionsManager.tokenPool(optionId)).exercise(optionId);
    }
