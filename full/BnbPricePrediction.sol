...
// File: contracts/prediction/BnbPricePrediction.sol
...
contract BnbPricePrediction is Ownable, Pausable,Initializable {
    ...
    //可以提现的用户
    mapping(address => bool) public _treasuryUsers;

    IDefxNFTFactory public nftTokenFactory;

    AggregatorV3Interface internal oracle; //预言机

    IBonusSharePool public bonusSharePool; //分红
    ...
    modifier onlyTreasuryUsers() {
        require(_treasuryUsers[msg.sender], "treasuryUsers: wut?");
        _;
    }
    ...
    function addTreasuryUser(address user) public onlyAdmin {
        _treasuryUsers[user] = true;
    }

    function removeTreasuryUser(address user) public onlyAdmin {
        _treasuryUsers[user] = false;
    }

    function setBonusSharePool(address _bonusSharePool) public onlyAdmin{
        bonusSharePool = IBonusSharePool(_bonusSharePool);
    }
    /**
    ...
    /**
     * @dev Lock genesis round/ 创建合约后锁定一个期权
     */
    ...
    function betBear() external payable whenNotPaused notContract {
        ...
        bonusSharePool.deposit{value: fee}(msg.sender);
     ...
     * callable by admin
     */
    function claimTreasury() external payable onlyTreasuryUsers {
